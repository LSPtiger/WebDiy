...
static pktbuf_t pktbuf_buffer[PKTBUF_BLK_CNT];
...
static inline curr_blk_tail_free(pktblk_t * blk){
...
static void display_check_buf(pktbuf_t * buf){
    ...
    for(curr = pktbuf_first_blk(buf); curr; curr = pktblk_blk_next(curr)){
        ...
        if((curr->data < curr->payload) || (curr->data >= curr->payload + PKTBUF_BLK_SIZE)){
            dbg_error(DBG_BUF, "wrong block data");
        }
        ...
        if(blk_total != PKTBUF_BLK_SIZE){
            dbg_error(DBG_BUF, "wrong block size: %d != %d", blk_total, PKTBUF_BLK_SIZE);
        }
...
static void pktbuf_insert_blk_list(pktbuf_t * buf, pktblk_t * first_blk, int add_list){
    if(add_list){
        while (first_blk)
        {
            ...
            nlist_insert_tail(&buf->blk_list, &first_blk->node);
...
static pktblock_free(pktblk_t * block){
    nlocker_lock(&locker);
...
net_err_t pktbuf_resize(pktbuf_t * buf, int to_size){
    ...
    if(buf->total_size == 0){
        ...
        if(!blk){
            dbg_error(DBG_BUF, "blk_list alloc failed.");
            return NET_ERR_MEM;
        }
        ...
        if(remain_size >= inc_size){
            ...
            if(!new_blocks){
                dbg_error(DBG_BUF, "new_blocks alloc failed.");
                return NET_ERR_MEM;
            }
        ...
        while (curr_blk)
        {
            ...
            nlist_remove_node(&buf->blk_list, &curr_blk->node);
...
net_err_t pktbuf_join(pktbuf_t * dest, pktbuf_t * src){
    ...
    dbg_assert(src->ref != 0, "src buf ref ==0");
    pktblk_t * first;
    while ((first = pktbuf_first_blk(src)))
    {
        nlist_remove_head(&src->blk_list);
...
net_err_t pktbuf_set_cont(pktbuf_t * buf, int size){
    ...
    if (size > PKTBUF_BLK_SIZE)
    {
        dbg_error(DBG_BUF, "size is too big :%d > %d ",size, PKTBUF_BLK_SIZE);
    }
...
net_err_t pktbuf_copy(pktbuf_t * dest, pktbuf_t * src, int size){
    dbg_assert(dest->ref != 0, "dest buf ref == 0");
    dbg_assert(src->ref != 0, "src buf ref == 0");
